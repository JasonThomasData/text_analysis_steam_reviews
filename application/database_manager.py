...
def create_steam_reviews(db_location):
...
def drop_steam_reviews(db_location):
    with sqlite3.connect(db_location, timeout=20) as db:
        cur = db.cursor()
        cur.execute('DROP TABLE steam_reviews;')

def insert_data_steam_reviews(db_location, url, app_num, date_scraped, classified, user_recommendation, user_review_text, user_name):
    with sqlite3.connect(db_location, timeout=20) as db:
        cur = db.cursor()
...
def remove_duplicates_steam_reviews(db_location):
    '''
...
def retrieve_steam_reviews(db_location, user_recommendation, classified, review_quantity):
    with sqlite3.connect(db_location, timeout=20) as db:
        cur = db.cursor()
...
def retrieve_last_steam_reviews(db_location):
    with sqlite3.connect(db_location, timeout=20) as db:
        cur = db.cursor()
